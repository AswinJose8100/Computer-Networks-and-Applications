...
void A_input(struct pkt packet)
{
  ...
  if (!IsCorrupted(packet)) {
    ...
    if (packet.acknum==var_temp_ack){    /**** 2. FILL IN CODE replace TRUE with test whether this is a new ACK ***/
      /* packet is a new ACK */
      if (TRACE > 0)
...
void B_input(struct pkt packet)
{
  ...
  if  ( (!IsCorrupted(packet))  && (packet.seqnum == expectedseqnum) ) {
    ...
    /* calling the fuction above to determine if the packet is corrupt. */
    if(IsCorrupted(packet))
    {
      sendpkt.acknum=-1;
    }
